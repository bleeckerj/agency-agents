...
#
# Reads integration files from integrations/ and copies them to the appropriate
# config directory for each tool. Some targets are generated, so run
# scripts/convert.sh first if a generated integration is missing or stale.
#
# Usage:
...
install_codex() {
  ...
  local count=0
  [[ -d "$src" ]] || { err "integrations/codex/skills missing from repo checkout."; return 1; }
  mkdir -p "$dest"
  local d
  while IFS= read -r -d '' d; do
    local name; name="$(basename "$d")"
...
main() {
  ...
  box_bot
  printf "\n"
  dim "  Run ./scripts/convert.sh to regenerate generated integrations after adding or editing agents."
  printf "\n"
}
